...
export class WebpageHoverProvider implements vscode.HoverProvider {
  ...
  private extract_url_at_position(document: vscode.TextDocument, position: vscode.Position): string | null {
  ...
  private async find_webpage_by_url(url: string): Promise<WebpageMetadata | null> {
    try {
      ...
      if (webpage) {
        ...
        const content = memory_results.length > 0 
          ? memory_results[0].value.pageContent.substring(0, 500) + '...'
  ...
  private create_hover(webpage: WebpageMetadata): vscode.Hover {
    ...
    if (webpage.visited_at) {
      ...
      markdown.appendMarkdown(`**Visited:** ${date.toLocaleDateString()} ${date.toLocaleTimeString()}\n\n`);
    }
