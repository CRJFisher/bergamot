...
**Plaintext side-channels, named honestly.** Two paths put metadata on disk unencrypted and are open gaps against this adversary, not covered by store encryption: the **dev replay ring** (`<storage_base>/captures/`) keeps a bounded ring of plaintext visit JSONs while dev logging is on; and **dev-mode logging** writes visit URLs to `dev-log.jsonl`. The right-to-forget cascade sweeps matching replay files, but a file present before a forget is forensically recoverable after deletion, and `dev-log.jsonl` is never rewritten. The visit inbox is no longer a plaintext gap — visits-in-flight are buffered inside the encrypted metadata store (`visit_inbox` table, task-39.8) and are covered by the cascade's DB sweep.
...
- **Cascading right-to-forget** (task-39.5): `right_to_forget.ts` deletes by URL, origin, or time range across the metadata record (including the `visit_inbox` buffer table, the fetch log by stored and post-redirect URL, referrer scrubbing on surviving visits, and empty-tree removal), the encrypted content cache, the plaintext dev-replay buffer (`captures/`), and the in-memory outcome ring; the live queue is purged first. Vector and cluster stores join the cascade as they land. Deletes are transactional per store and idempotent across stores, derived-content-first; cross-store atomicity over separate database files is not possible — a failure between stores can only leave metadata without content (the safe direction), and re-running the forget completes it. What survives a forget: `dev-log.jsonl` lines, and the forensic recoverability of any plaintext replay file that existed before the forget.
