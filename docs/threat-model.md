...
4. **The re-downloaded content cache** (`content_cache.db`) — on-demand, encrypted, quarantined page content fetched from stored public URLs; populated only when a consumer opts in with a named scope (consumers: the right-to-forget cascade via the `Bergamot: Forget` command; TDT, task-36.3, when it lands).
...
- **Cascading right-to-forget** (task-39.5): bounds the blast radius of every asset — `right_to_forget.ts` deletes by URL, origin, or time range across the metadata record (including referrer scrubbing on surviving visits and empty-tree removal) and the encrypted content cache; vector and cluster stores join the cascade as they land. Deletes are transactional per store and idempotent across stores, derived-content-first.
