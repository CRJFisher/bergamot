...
2. **Never capture private browsing.** The extension is disabled in incognito / private-browsing windows (Chrome manifest `"incognito": "not_allowed"`), so no listener fires and no capture request is ever made for such a tab — no metadata is recorded for it. The user has explicitly signalled "do not remember this," and that signal is honoured absolutely.
...
4. **Right-to-forget is real and cascading.** Deleting — by URL, by origin, or by time-range — removes the metadata row AND every derived artifact: any encrypted re-downloaded content cache, all derived vectors, and all cluster memberships. Deletion is atomic per store; across stores the cascade is ordered derived-content-first and idempotent, so no derived artifact can survive its metadata (amendment: docs/decisions/principle-4-per-store-atomicity.md). Forgetting is deletion, not hiding: no derived artifact may continue to encode forgotten content. This primitive is part of the core spine, not deferred.
