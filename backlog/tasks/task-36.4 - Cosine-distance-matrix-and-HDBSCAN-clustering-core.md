...
- **`tdt` ships a pure-JS backend floor; the native backend lives only in the `vscode` host.** `tdt` is a pure, environment-agnostic library, so it depends only on `clustering-tfjs`, the pure-JS `@tensorflow/tfjs` (the always-available `'cpu'` backend floor), and `@tensorflow/tfjs-core` (to read the active backend name) — never the native `@tensorflow/tfjs-node`. Because a backend is always present, clustering degrades gracefully to the slower vanilla `'cpu'` path anywhere rather than failing; `algo_version` records that distinctly. The native `@tensorflow/tfjs-node` backend is declared by `vscode` (guaranteed Node) and the library's auto-probe prefers it when present (~11×); `tdt`'s own tests pick it up via workspace hoisting.
...
- **Dependency-absent fails clearly; backend-absent degrades gracefully (AC#6).** `require_hdbscan` gives a TDT-framed error if the `clustering-tfjs` export is missing. A *backend* is never absent: the `@tensorflow/tfjs` floor means an unavailable native backend silently falls back to the vanilla `'cpu'` path instead of throwing, so there is no backend-absent error to reframe.

### Verification

71 `@bergamot/tdt` unit tests pass, including: the distance-matrix shape/zero-diagonal/symmetry/clamp invariants and the identical/orthogonal/opposite 0/1/2 mapping; the page-count guard and the dimension-uniformity guard; a real-library `fit` producing ≥2 clusters with honest `-1` noise, `[0,1]` probabilities (0 for noise), and one exemplar per cluster; a bitwise-reproducibility test asserting identical `labels`, `probabilities`, and `exemplar_indices` across two fits; `algo_version` recording the version and the active backend; and the `require_hdbscan` dependency-absence guard. The `vscode` production bundle (`scripts/bundle.mjs`) and the externals guard (`scripts/check-bundle-externals.mjs`) both pass, confirming no TensorFlow chain leaks into the bundle.
