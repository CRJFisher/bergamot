...
<!-- SECTION:NOTES:BEGIN -->
Deferred verification carried from task-31.5 (clean ingestion, landed 2026-06-10): once the harness exists, confirm that heuristic main-content extraction (Defuddle markdown, replacing raw HTML as the embedded/retrieved body) shows neutral-or-better retrieval quality on the golden dataset — task-31.5 AC#3, which could not be measured before this harness. If extraction regresses retrieval, revisit the extractor (e.g. Defuddle options) rather than reverting to raw HTML.
<!-- SECTION:NOTES:END -->
