...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Ingestion makes ZERO LLM calls; capture is fully deterministic and offline (no auth / prompt-length / non-JSON failure surface) — asserted via an injected jest.fn() LLM mock showing 0 calls for any captured page
- [ ] #2 The raw page is stored losslessly (zstd-compressed BLOB) in DuckDB webpage_capture keyed by page_session_id, as the durable source of truth for downstream extraction/RAG; compress->store->decompress is byte-identical
- [ ] #3 Cheap metadata (title, site_name, author, published_at, lang) is read non-LLM from the HTML <head>/og: tags — no main-content extraction at ingest
- [ ] #4 The LLM classifier, content-processing, analysis (title/summary/intentions) and tree-intentions calls are all deleted; intentions and tree_intentions are removed from schema and types; navigation trees still build (url, title) with those fields absent
- [ ] #5 A deterministic relevance gate (content size / link-density / content-type) replaces the knowledge-only classifier; PDFs and non-HTML are dropped with a specific reason
- [ ] #6 All extraction / chunking / embedding / summarisation moves to the RAG-prep pipeline (task-31); the raw page is the parent-document source so task-31.3 is not blocked
- [ ] #7 Haiku is the explicit configurable default (bergamot.models.fast) for the pipeline's model use; the 'smart' role key is reserved but not user-exposed
- [ ] #8 All schema changes are destructive dev-DB resets (no ALTER/backfill, no shims); a documented reset procedure exists and every schema-changing subtask references it
- [ ] #9 simple_workflow.ts / WebpageWorkflow and related vestigial names are renamed; no stateful workflow class remains; all callers updated with no aliases
- [ ] #10 A light capture eval asserts raw-page round-trip (lossless) and metadata correctness; there is no LLM eval at ingest
<!-- AC:END -->
