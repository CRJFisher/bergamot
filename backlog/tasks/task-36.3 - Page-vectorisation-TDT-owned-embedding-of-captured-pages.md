...
This task ships the pure library, fakes, and tests. Its production integration is **TASK-36.3.1**: the concrete DuckDB `topic_page_vector` `VectorStore`, the production local embedder (`bge-small-en-v1.5` via `@huggingface/transformers`), the batched embed pass that calls `build_page_vector` over the cached public corpus **ahead of a clustering run** (off the extension-host event loop) so vectors accumulate before TDT needs them, and the right-to-forget cascade entry for the store. The clustering orchestration (`run_tdt`: dedupe → read vectors → distance matrix → cluster) lands in the later distance/representation/trigger subtasks and only **reads** the vectors the embed pass produced.
...
- **Embedding runs as a batched pass ahead of a TDT clustering run, not per-page on the capture hot path.** TDT owns the page representation + embedding model (the `build_page_vector` definition). The canonical page vector is produced by a batched embed pass that reads each page's already-extracted public content from the durable content cache (warmed by the eager re-download at capture time) and vectorises every page missing a current-model vector via `resolve_page_vector` build-on-miss, off the extension-host event loop. Because the content is durably cached, embedding does not need to happen at ingestion — keeping the capture path free of CPU-bound work and the embedding model resident only while a pass runs. TDT (and RAG) only read the stored vectors. The pure library shipped here is orchestration-agnostic — this is purely about who calls it and when; the concrete store, the production local embedder, the batched embed pass, and the cascade entry are TASK-36.3.1.
