...
This task ships the pure library, fakes, and tests. Its production integration is **TASK-36.3.1**: the concrete DuckDB `topic_page_vector` `VectorStore`, the wiring that calls `build_page_vector` at **ingestion** (in the same eager-re-download pass, off the capture hot path) so a page's vector is produced once when it reaches the extension, and the right-to-forget cascade entry for the store. The clustering orchestration (`run_tdt`: dedupe → read vectors → distance matrix → cluster) lands in the later distance/representation/trigger subtasks and only **reads** the vectors ingestion produced.
...
- **Embedding runs at ingestion, not on TDT's schedule.** TDT owns the page representation + embedding model (the `build_page_vector` definition), but the canonical page vector is produced once when a page reaches the extension — in the same eager-re-download pass that warms the content cache, off the capture hot path — and stored in `topic_page_vector`. TDT (and RAG) only read it; `resolve_page_vector`'s build-on-miss is the backfill / alternate-model-or-repr path, not the common case. The pure library shipped here is orchestration-agnostic — this is purely about who calls it; the ingestion wiring + concrete store is TASK-36.3.1.
